...
function normalizeResults(payloads) {
  return payloads.flatMap((payload) => {
    ...
    return payload.freeSlots.map((slot) => ({
      ...
      bookingUrl,
      bookingDateHint: payload.bookingDateHint || "",
      city,
      district,
      mapQuery,
...
function renderResultRows(results) {
  ...
  results.forEach((result) => {
    ...
    const bookingLink = fragment.querySelector(".result-link-inline");
    bookingLink.href = result.bookingUrl;
    if (result.bookingDateHint) {
      bookingLink.textContent = `Buchen ${formatDateShort(result.date)}`;
      bookingLink.title = result.bookingDateHint;
      bookingLink.setAttribute("aria-label", result.bookingDateHint);
    }
